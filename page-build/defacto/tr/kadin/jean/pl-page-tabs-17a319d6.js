    ...
    connectedCallback() {
      ...
      window.pageStore.subscribe(
        ...
        state => {
          ...
          this.update({ selected_pl_tab });
        }
      );
    }

    update({ selected_pl_tab }) {
      if (selected_pl_tab === name) {
        console.log('I am clicked', name);
        this.classList.add('border-bottom');
        this.classList.add('border-4');
        this.classList.add('border-primary');
        this.document.getElementById(name).classList.remove('text-secondary');
        this.document.getElementById(name).classList.add('text-primary');
      } else {
        this.classList.remove('border-bottom');
        this.classList.remove('border-4');
        this.classList.remove('border-primary');
        this.document.getElementById(name).classList.add('text-secondary');
        this.document.getElementById(name).classList.remove('text-primary');
      }
    }
  }
);
