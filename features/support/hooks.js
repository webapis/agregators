...
Before(async function (scenario) {
  ...
  if (order === 0) {
    ...
    //load backend data from file
    const backEndBefore = fs.readFileSync(`${process.cwd()}/mock-data/back-end/${(order - 1).toString()}-before.json`, { encoding: 'utf-8' })
    
    // upload backend data
    await nodeFetch({ host: process.env.databaseHost, path: `/.json?auth=${idToken}`, method: 'PUT', body: JSON.stringify(backEndBefore), headers: {}, port: process.env.dbPort, ssh: process.env.dbSsh })
    //load data for local storage
    const localStorageBefore = fs.readFileSync(`${process.cwd()}/mock-data/local-storage/${(order - 1).toString()}-before.json`, { encoding: 'utf-8' })
...
After(async function (scenario) {
    ...
    fs.writeFileSync(`${process.cwd()}/mock-data/local-storage/${name}-after.json`, localStorageAfter, { encoding: 'utf-8' })
