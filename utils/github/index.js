...
async function fetchGithubAuthCode(res) {
    const url = `https://github.com/login/oauth/authorize?client_id=198fd462ac295507b855&redirect_uri=http://localhost:3000/user-settings.html&scope=repo&state=${res.uid}&allow_signup=true`
...
async function fetchGithubAccessToken(code, res) {
    ...
    userRef.update({ ghtoken: 
        access_token }, (error) => {
        if (error) {
            console.log('error', error)
            res.send('Error happened')
        } else {
            const filepath = `${process.cwd()}/src/${process.env.app}/user-settings.html`;
            debugger;
            res.setHeader('Content-Type', 'text/html');
