        ...
        window.FB_DATABASE.ref(`workspaces/${workspaceName}/workflowInitials/tasks/${taskId}/workflows`).get((error, result) => {
            if (result) {
                ...
                workflows && workflows.forEach(wf => {
                    ...
                    document.getElementById('workflows').insertAdjacentHTML('beforeend', `<div class="list-group-item d-flex justify-content-between "> <a href="#" class="nav-link" id="${workflowName}-workflow-editor-btn">${workflowName}   ||  ${workflowDescription}</a>  <a href="#" id="${workflowName}-workflow-config-btn" ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-gear" viewBox="0 0 16 16">
                    ...
                    document.getElementById(`${workflowName}-workflow-config-btn`).addEventListener('click', (e) => {
                        e.preventDefault()
                        window.pageStore.dispatch({ type: window.actionTypes.WORKFLOW_SELECTED, payload: { workflowName, workflowKey, workflowDescription, repoName } })
                        window.location.replace('/workflow-configuration.html')
                    })
                    document.getElementById(`${workflowName}-workflow-editor-btn`).addEventListener('click', (e) => {
                        e.preventDefault()
                        try {
                            
                
                       debugger;
                            window.pageStore.dispatch({ type: window.actionTypes.EDIT_WORKFLOW, payload: { workflowName, workflowKey, workflowDescription, selectedRepo: repoName, ownersRepos: [], loading: false,selectedBranch } })
